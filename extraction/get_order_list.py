...
# def get_order_list(partner_id, partner_key, shop_id, access_token, date_str):
#     hora_inicio = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#     print("Coletando pedido de", date_str)
#     timestamp = int(time.time())
#     path = '/api/v2/order/get_order_list'
#     time_from = date_to_timestamp(date_str)
#     time_to = time_from + 2 * 86400 # Cobre 2 dias
...
#     tempo_total = datetime.strptime(hora_fim, '%Y-%m-%d %H:%M:%S') - datetime.strptime(hora_inicio, '%Y-%m-%d %H:%M:%S')
#     print(f"Processo de extração de pedidos finalizado em {tempo_total}")
#     return [f"SHP-{order['order_sn']}" for order in all_orders]