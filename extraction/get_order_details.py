...
# def get_order_details(partner_id, partner_key, shop_id, access_token, order_sn_list):
#     hora_inicio = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#     print("Coletando detalhes de", len(order_sn_list), "pedidos")
#     path = '/api/v2/order/get_order_detail'
#     all_details = []
...
#             response_json = response.json()
#             for detail in response_json.get('response', {}).get('order_list', []):
#                 detail['order_sn'] = f"SHP-{detail['order_sn']}"
#             all_details.extend(response_json.get('response', {}).get('order_list', []))
#         else:
#             print("Error:", response.status_code, response.text)
#             break
